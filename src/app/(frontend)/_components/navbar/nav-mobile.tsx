...
const NavMobile: React.FC<NavProps> = ({ logoUrl }) => {
    ...
    <motion.nav className="relative h-full text-[#272727] bg-background">
      <ul className="h-full py-4 px-4 flex items-center justify-between text-lg">
        ...
        <li>
          ...
          {open && (
            <motion.button onClick={() => setOpen(false)} className="flex items-center">
              <CircleX />
            </motion.button>
          )}
        </li>
      ...
      {open && (
        <motion.ul
          className="absolute top-full left-0 w-full h-[calc(100vh-4rem)] bg-background flex flex-col justify-start items-center text-lg font-semibold gap-8"
          layoutId="navContent"
          onClick={onClickLink}
        >
          {' '}
          ...
          <li className="w-full text-center">
            <Link
              href="/menu"
              className={`block w-full ${pathname == '/menu' ? 'text-primary' : ''}`}
            >
              Menu
            </Link>
          </li>
          <li className="w-full text-center">
            <Link
              href="/about"
              className={`block w-full ${pathname == '/about' ? 'text-primary' : ''}`}
            >
              About
            </Link>
          </li>
